...
class BranchBloc extends Bloc<BranchEvent, BranchState> {
  BranchBloc() : super(LocationsInitialState()){
    on<SetLocationsEvent>((event, emit)async{
      if(event.useDeviceLocation){
        Position? position = await determineLastPostion();
        LatLng currentLocation = LatLng(position!.latitude, position.longitude);
        emit(LocationLoadedState(userLocation:currentLocation, destination: event.branchLocation, destinationName:event.branchName,));
      }else{
